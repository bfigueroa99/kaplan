...
\begin{spanishbox}[title=Para Hispanohablantes: Prepositions IN/AT/ON for Places]
⚠️ \textbf{El español "EN" se divide en 3 preposiciones en inglés:}

\begin{itemize}
    \item \textbf{IN} = dentro de espacios cerrados/ciudades/países
    \begin{itemize}
        \item ✓ IN London, IN the house, IN Spain
        \item ❌ No: AT London, ON the house
    \end{itemize}
    \item \textbf{AT} = puntos específicos/direcciones/eventos
    \begin{itemize}
        \item ✓ AT home, AT the theatre, AT 21 Oxford Street
        \item ❌ No: IN home, ON the theatre
    \end{itemize}
    \item \textbf{ON} = superficies/calles (sin número)
    \begin{itemize}
        \item ✓ ON the table, ON Oxford Street
        \item ❌ No: IN the table, AT Oxford Street
    \end{itemize}
\end{itemize}

💡 \textbf{Truco:} Si tiene número de casa = AT. Si solo nombre de calle = ON.
\end{spanishbox}
