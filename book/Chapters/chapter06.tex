...
\begin{britishbox}
\textbf{Comparing British Cities:} London is \textbf{the largest} and \textbf{most expensive} city in the UK. Edinburgh is \textbf{more historic} and \textbf{hillier than} most British cities. Manchester is \textbf{wetter than} London but \textbf{cheaper}. Brighton is \textbf{the sunniest} seaside town. Oxford and Cambridge are both \textbf{older and more prestigious} than many universities worldwide. British people often say \textit{"It's not as bad as people think"} (= quite good) when describing their cities—typical British understatement!
\end{britishbox}

\begin{comparisonbox}[title=🇪🇸 Comparativos en Español vs Inglés]
\textbf{⚠️ Diferencias importantes:}

\textbf{1. "Más" vs "-er":}
\begin{itemize}
    \item Español: SIEMPRE "más" → "más grande", "más pequeño"
    \item Inglés: Depende de la longitud del adjetivo
    \begin{itemize}
        \item Corto (1-2 sílabas): \textbf{bigger, smaller, faster}
        \item Largo (3+ sílabas): \textbf{more expensive, more beautiful}
    \end{itemize}
\end{itemize}

\textbf{2. "Que" = "THAN" (NO "that"):}
\begin{itemize}
    \item Español: "Londres es más grande \textit{que} París"
    \item English: "London is bigger \textbf{than} Paris"
    \item \textcolor{red}{❌} "bigger that" (error común de hispanohablantes)
\end{itemize}

\textbf{3. Superlativo necesita "THE":}
\begin{itemize}
    \item \textcolor{red}{❌} "London is most expensive city"
    \item \textcolor{green}{✓} "London is \textbf{the} most expensive city"
\end{itemize}
\end{comparisonbox}
